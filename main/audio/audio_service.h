...
#define AS_EVENT_LOCAL_ASR_RUNNING          (1 << 4)
...
public:
    ...
    void ResetDecoder();
    void SetLocalAsrCallback(std::function<void(std::vector<int16_t>&&)> callback) {
        on_local_asr_data_ = callback;
    }
    void EnableLocalAsr(bool enable);
...
private:
    ...
    std::function<void(std::vector<int16_t>&&)> on_local_asr_data_;
