...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        /* Feed the local ASR (offline command recognition) */
        if (bits & AS_EVENT_LOCAL_ASR_RUNNING && on_local_asr_data_) {
            std::vector<int16_t> data;
            // Use a reasonable audio chunk size for local ASR feeding
            // The actual multinet chunk size is handled inside LocalAsr
            int samples = 960; // ~60ms at 16kHz, common for ESP-SR
            if (ReadAudioData(data, 16000, samples)) {
                // If input channels is 2, extract mono (left channel)
                if (codec_->input_channels() == 2) {
                    auto mono_data = std::vector<int16_t>(data.size() / 2);
                    for (size_t i = 0, j = 0; i < mono_data.size(); ++i, j += 2) {
                        mono_data[i] = data[j];
                    }
                    on_local_asr_data_(std::move(mono_data));
                } else {
                    on_local_asr_data_(std::move(data));
                }
                continue;
            }
        }
...
void AudioService::EnableLocalAsr(bool enable) {
    ESP_LOGI(TAG, "%s local ASR", enable ? "Enabling" : "Disabling");
    if (enable) {
        xEventGroupSetBits(event_group_, AS_EVENT_LOCAL_ASR_RUNNING);
    } else {
        xEventGroupClearBits(event_group_, AS_EVENT_LOCAL_ASR_RUNNING);
    }
}
